function evaluate_similarity_polygon_rulclipper
%% Estimate Remaining Useful Life / make forecast / prognostics using |RULCLIPPER| algorithm
% Assumption: We have a training datasets made of training instances, which are matrices
% with lines 1:T (length of time series) and column representing features or sensors values.
% From those datasets, we have estimated an indicator signal, call health indicator,
% generally starting from 1 (good health) and falling to 0 gradually (until failure).
% We now have a testing instance known from 1 to t, when does the time series will go below 0 ?
% Several approaches for making prognostics, prediction, forecasting... one of them is called
% "similarity based approaches" [2]. One method proposed in [1] called RULCIPPER standing for
% "Remaining Useful Life estimation based on impreCise heaLth Indicator modeled by
% Planar Polygons and similarity-basEd Reasoning" and is implemented here.
% So the idea is to find the "most similar" instances within a training dataset
% made of trajectories (time-series) to a testing instance partially observed
% (from 1:t, we want t+1:t+H).
% In RULCLIPPER, the similarity is computed by 1) considering instances as no
% longer signals but polygons, 2) making use of polygon intersection between
% test and training instances and 3) compute recall, precision and F1-measure
% to quantify the similarity.
%
% This file is dedicated to the application of RULCLIPPER onto CMAPSS
% datasets (see my Matlab central account for more details).
% Make the necessary adaptation for a use on other datasets.
% Please see the paper [1] for more details, in particular to understand
% some of the "rules" included and related to CMAPSS datasets.
%
% This code is able to reproduce results for table 5-6 in paper [1]. For table 7 you should
% implement a "fusion" between results obtained with different subsets of
% features. So i provide also the function health_indicators_estimation_cmapss.m to compute
% each pair of subsets for the turbofan datasets. One of the fusion rule used in the paper
% is also implemented.
% If you want to obtain the indicators/models for other subsets please
% contact me, i can send matlab.csv files.
% I do not provide here the models for the PHM data challenge, only CMAPSS. You should
% create you own model and then submit your results to the NASA PCOE website.
%
...
disp('This gives an example on how to apply RULCLIPPER')
disp('Illustrated on CMAPSS')
disp('Only testing instance is used here, feel free to make a loop on all datasets')

clear all

datasetNumber = input('Which dataset ? (#1,#2,#3,#4): ')
it_donnee_test = input('Which testing instance ? (#1=1:100,#2:1:248,#3:1:100,#4:1:249): ')

%%%%%%%%%%%%%%%%%%%%%%%%%%%
% HERE LOAD THE DATA PLEASE HAVE A LOOK IN THE FOLDERS I HAVE PROVIDED
% I give here only some example in the "SINGLE" directory. Please have a look on "MULTIPLE".
% This loads two cells: responses_models and data_train_modele_morceaux,
% see readme.txt.
if datasetNumber==1
    %load 'SINGLE/data_indicators_turbofan1_9_10_14_25_26_singleSubsetTable5.mat'
    [responses_models, data_train_modele_morceaux, ~, directoryData] = health_indicators_estimation_cmapss(1, [7 8 12 16 17 20 9 10 14 25 26], 1, true);
elseif datasetNumber==2
    %load 'SINGLE/data_indicators_turbofan2_9_13_25_singleSubsetTable6.mat'
    [responses_models, ~, data_train_modele_morceaux, directoryData] = health_indicators_estimation_cmapss(2, [7 8 12 16 17 20 9 13 25], 6, false);
elseif datasetNumber==3
    %load 'SINGLE/data_indicators_turbofan3_9_13_14_22_26_singleSubsetTable5.mat'
    [responses_models, data_train_modele_morceaux, ~, directoryData] =health_indicators_estimation_cmapss(3, [7 8 12 16 17 20 9 13 14 22 26], 1, true);
elseif datasetNumber==4
    %load 'SINGLE/data_indicators_turbofan4_9_10_11_22_singleSubsetTable6.mat'
    [responses_models, ~, data_train_modele_morceaux, directoryData] =health_indicators_estimation_cmapss(4, [7 8 12 16 17 20 9 10 11 22], 6, false);
else error('Bad number for dataset');
end

% Use true to display some images
FIGPOL = input('Do you want to display images ? (1:yes, 0:no): ');
if FIGPOL~=1 && FIGPOL~=0, FIGPOL=0, end %error('Bad answer (1 or 0 allowed)'), end

% Those are the RULs defined in the datasets, you can download it on PCOE's website
try, rul1 = dlmread(sprintf('%s/RUL_FD00%d.txt',directoryData,datasetNumber));
catch, error(sprintf('File RUL_FD00%d.txt not found',datasetNumber))
end

% In this example, i use only one test
...
elseif datasetNumber==2
    ...
    % This rule is the one of table 6, line 1
    r6 = [r(1:q) Z];    
    ...
    % in the paper it is written mean but it is a median actually
...
disp('Now loop over all testing data')
disp('for data set #1, you should obtain around s=272 and accuracy=68')
disp('for data set #2, you should obtain around s=3300 and accuracy=47')
% see tables 5-7 in the paper
%
% the solution is something like this:
% lesRul =[];
% for all testing data
%   ... % do the previous lines, see above
%   lesRul = [lesRul ; RULestimated];% memory
%   ...
% endfor
% evaluate
% [S FPR FNR MAPE MAE MSE Acc] = prognostics_metrics(rul1, lesRul,13,10);
% sum(S), Acc, FPR, FNR, MAPE, MAE, MSE, Acc
